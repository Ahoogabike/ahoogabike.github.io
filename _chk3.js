  ...
  if(toBook.length){
    ...
    try{
      ...
      for(const so of toBook){
        ...
        if(!dry){
          // pin the DSV label to the Deco Coat P0 receipt (the incoming picking of the PO that references this SO)
          let attachId = res.pickingId;   // legacy: an internal transfer, if one was ever created
          if(!attachId){ attachId = await poReceiptIdForSO(so); if(attachId) res.receiptId = attachId; }
          if(attachId){
            try{ await odoo('stock.picking','write',[[attachId],{carrier_tracking_ref:String(bk.bookingId)}]); }catch(e){}
            if(bk.labelBase64){ try{ await odoo('ir.attachment','create',[{name:'DSV-'+bk.bookingId+'.pdf',type:'binary',mimetype:'application/pdf',datas:bk.labelBase64,res_model:'stock.picking',res_id:attachId}]); }catch(e){} }
            try{ await odoo('stock.picking','message_post',[[attachId]],{body:'DSV '+(bk.draft?'draft':'booking')+' '+bk.bookingId+(bk.trackingUrl?' \u00b7 '+bk.trackingUrl:''),message_type:'comment',subtype_xmlid:'mail.mt_note'}); }catch(e){}
          }
        }
...
/* ── STEP C: DSV label ── */
/* find the Deco Coat P0 receipt picking (incoming) for a sales order — used to pin the DSV label to it */
async function poReceiptIdForSO(so){
  try{
    const pol=await odoo('purchase.order.line','search_read',[['|',['name','ilike',so],['order_id.origin','ilike',so]]],{fields:['order_id'],limit:1});
    if(!(pol&&pol.length&&Array.isArray(pol[0].order_id))) return null;
    const poName=pol[0].order_id[1];
    const picks=await odoo('stock.picking','search_read',[[['origin','=',poName],['picking_type_code','=','incoming']]],{fields:['id'],order:'id desc',limit:1});
    return (picks&&picks.length)?picks[0].id:null;
  }catch(e){ return null; }
}
